    ...
    if (Object.keys(pawnsToPerformElPassant).length > 0) {
        if (xAxisIndex - 1 >= 0) {
            ...
            if (
                ...
                let boxForElPassant = document.getElementById(
                    `${xAxis[xAxisIndex - 1]}-${yAxis[yAxisIndexForCapture]}`
                );
        ...
        if (xAxisIndex + 1 < xAxis.length) {
            let rightBox = document.getElementById(
                `${xAxis[xAxisIndex + 1]}-${yAxisPos}`
            );
            ...
            if (yAxisIndex + yInc < yAxis.length && xAxisIndex - xInc > -1) {
                if (!topLeftCollision) {
                    let topLeftBlock = document.getElementById(
                        `${xAxis[xAxisIndex - xInc]}-${
                            yAxis[yAxisIndex + yInc]
                        }`
                    );

                    if (topLeftBlock.childElementCount > 0) {
                        if (
                            topLeftBlock.children[0].classList.contains(enemy)
                        ) {
            ...
            if (
                ...
                if (!topRightCollision) {
                    ...
                    if (topRightBlock.childElementCount > 0) {
                        if (
                            topRightBlock.children[0].classList.contains(enemy)
                        ) {
            ...
            if (yAxisIndex - yInc > -1 && xAxisIndex - xInc > -1) {
                if (!bottomLeftCollision) {
                    ...
                    if (bottomLeftBlock.childElementCount > 0) {
                        if (
                            bottomLeftBlock.children[0].classList.contains(
                                enemy
                            )
                        ) {
            ...
            if (yAxisIndex - yInc > -1 && xAxisIndex + xInc < xAxis.length) {
                if (!bottomRightCollision) {
                    ...
                    if (bottomRightBlock.childElementCount > 0) {
                        if (
                            bottomRightBlock.children[0].classList.contains(
                                enemy
                            )
                        ) {
...
const isPawnAtTheEndOfTheBoard = (currentPlayer, pawnPosition) => {
    ...
    if (currentPlayer === "light") {
        let positionIndex = lightPiecesEndingPosition.findIndex(
        ...
        let positionIndex = blackPiecesEndingPosition.findIndex(
            (pos) => pos === pawnPosition
        );
