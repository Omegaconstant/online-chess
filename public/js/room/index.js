    ...
    if (boxToMove.children.length > 0) {
        ...
        if (!isLeftCastlingPerformed || !isRightCastlingPerformed) {
            if (piece.dataset.piece === "rook") {
                ...
                let pieceXAxisIndex = xAxis.findIndex(
                    (x) => x === currentBox.id[0]
                );
                let myKingXAxisIndex = xAxis.findIndex(
                    (x) => x === myKingPosition[0]
                );
...
const moveEnemy = (move, pawnPromotion = null, elPassantPerformed = false) => {
    ...
    if (elPassantPerformed) {
        ...
        if (player === "light") {
            capturedPieceBox = document.getElementById(
                `${to[0]}-${parseInt(to[2]) - 1}`
            );
        } else {
            capturedPieceBox = document.getElementById(
                `${to[0]}-${parseInt(to[2]) + 1}`
...
const checkForElPassant = (enemyMove) => {
    ...
    if (player === "light") {
        enemyPawn = blackPieces.find(
            (enemyPiece) =>
                enemyPiece.piece === "pawn" && enemyPiece.position === from
        );
    } else {
        enemyPawn = lightPieces.find(
            (enemyPiece) =>
                enemyPiece.piece === "pawn" && enemyPiece.position === from
        );
    }
